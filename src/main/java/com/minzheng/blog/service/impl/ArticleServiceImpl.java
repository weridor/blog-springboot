...
//        List<ArticleHomeDTO> articleDTOList = (List<ArticleHomeDTO>) operations.get("article_" + current);
        // 转换页码分页查询文章
//        if (org.springframework.util.CollectionUtils.isEmpty(articleDTOList)) {
        List<ArticleHomeDTO> articleDTOList = articleDao.listArticles((current - 1) * 10);
            // 将数据库取回的数据设置到Redis中
//            operations.set("article_"+current,articleDTOList);
//        }
    ...
    @Override
    public Map listsss(Long current) {
        Map<String, Object> map = new HashMap<>();
        ArrayList<Tag> tags = new ArrayList<>();
        tags.add(new Tag(156,"tag",new Date()));
        map.put("datas", tags);
        return map;
    }
